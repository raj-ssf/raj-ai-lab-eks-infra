...
resource "helm_release" "external_dns" {
  ...
  values = [
    yamlencode({
      ...
      # Phase 2 (Cilium migration): 'gateway-httproute' source dropped
      # because Gateway API CRDs aren't installed yet. Cilium's gatewayAPI
      # helm flag enables the controller but expects upstream Gateway API
      # CRDs to be pre-installed; that bootstrap belongs in Phase 3 when
      # we wire up shared-gateway. Until then, external-dns watches
      # only 'service' (for LoadBalancer Service DNS).
      sources = [
        "service",
      ]
      registry = "txt"
