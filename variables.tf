...
# Phase #69: alertmanager paging receivers. All optional with empty
# defaults — when unset, alertmanager continues to behave as
# before (alerts route through the pipeline but sink to "null"
# because the receivers' webhook_configs / pagerduty_configs lists
# remain empty). Set in terraform.tfvars (gitignored per the
# secrets-in-tfvars convention) to enable real paging.
#
# The Slack URL must be an Incoming Webhook URL from a Slack app
# configured in the destination workspace. The PagerDuty routing
# key is a service-integration key (Events API v2 format).

variable "slack_webhook_critical_url" {
  type        = string
  description = "Slack incoming-webhook URL for severity=critical alerts. Empty (default) means criticals sink to null."
  sensitive   = true
  default     = ""
}

variable "slack_webhook_warning_url" {
  type        = string
  description = "Slack incoming-webhook URL for severity=warning alerts. Empty (default) means warnings sink to null."
  sensitive   = true
  default     = ""
}
...
variable "pagerduty_routing_key" {
  type        = string
  description = "PagerDuty Events API v2 routing key for severity=critical alerts. Used in addition to slack_webhook_critical_url; both can be set so criticals page AND chat. Empty (default) disables PagerDuty."
  sensitive   = true
  default     = ""
}